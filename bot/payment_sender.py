...
async def check_subscriptions(logger=logger):
    ...
    for user in expired_and_active_users:
        pay_url = f"https://yoomoney.ru/quickpay/shop?amount=999&receiver=ТВОЙ_ЮКАССА_ID&label={user.telegram_id}"
        logger.info(f"Пользователю {user.telegram_id} отправляем оповещение, ссылка на оплату: {pay_url}")
        try:
            # Отправляем ссылку на оплату
            message = await get_payment_message()
            await bot.send_message(user.telegram_id, f"{message}\n{pay_url}")
